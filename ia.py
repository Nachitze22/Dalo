...
class IA:
    def __init__(self, app):
        ...
        self._modelo_odio_no_disponible = False
    ...
    def clasificar_imagen_sync(self, producto):
        ...
        img = variables_globales.obtener_transform()(img).unsqueeze(0)
        with torch.no_grad():
            outputs = self.modelo(img)
        _, predicted = outputs.max(1)
        etiqueta = variables_globales.obtener_classes()[predicted.item()].lower()
        return etiqueta
    ...
    def cargar_modelo_odio_async(self):
        if self.modelo_odio is not None or self.modelo_odio_cargando or self._modelo_odio_no_disponible:
            return
        ...
        def cargar():
            try:
                ...
                self._modelo_odio_no_disponible = True
            finally:
                self.modelo_odio_cargando = False

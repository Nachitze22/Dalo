...
class ProductosDB:
    ...
    def obtener_por_categoria(self, categoria):
        ...
        cursor.execute("""
            SELECT *
            FROM productos
            WHERE categoria = ? AND COALESCE(activo, 1) = 1
        """, (categoria,))
        filas = cursor.fetchall()
        conn.close()
        return [dict(fila) for fila in filas]
    ...
    def usuario_marco_util(self, opinion_id, usuario_id):
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT 1 FROM opinion_utiles
            WHERE opinion_id = ? AND usuario_id = ?
        """, (opinion_id, usuario_id))
        fila = cursor.fetchone()
        conn.close()
        return fila is not None
    ...
    def marcar_util(self, opinion_id, usuario_id):
        ...
        if ya_marcado:
            cursor.execute("""
                DELETE FROM opinion_utiles
                WHERE opinion_id = ? AND usuario_id = ?
            """, (opinion_id, usuario_id))
        else:
            cursor.execute("""
                INSERT INTO opinion_utiles (opinion_id, usuario_id)
                VALUES (?, ?)
            """, (opinion_id, usuario_id))
        conn.commit()
        conn.close()
    ...
    def descontar_stock(self, producto_id, cantidad):
        ...
        if nuevo_stock <= 0:
            cursor.execute("""
                UPDATE productos
                SET stock = 0, vendidos = COALESCE(vendidos, 0) + ?, activo = 0
                WHERE id = ?
            """, (cantidad, producto_id))
        else:
            cursor.execute("""
                UPDATE productos
                SET stock = ?, vendidos = COALESCE(vendidos, 0) + ?
                WHERE id = ?
    ...
    def obtener_pedidos_usuario(self, usuario_id):
        ...
        cursor.execute("""
            SELECT pe.id, pe.producto_id, pe.cantidad, pe.precio_unitario, pe.numero_orden, pe.fecha_creacion,
                   pe.entrega_tipo, pe.direccion_envio,
                   p.nombre, p.imagen, p.moneda
            FROM pedidos pe
...
# ================= Lista de deseos =================
    ...
    def toggle_deseo(self, usuario_id, producto_id):
        ...
        Devuelve True si quedó agregado, False si quedó eliminado."""
    ...
    def obtener_ids_deseos(self, usuario_id):
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT producto_id FROM lista_deseos
            WHERE usuario_id = ?
            ORDER BY fecha_creacion DESC
        """, (usuario_id,))
        ids = [fila[0] for fila in cursor.fetchall()]
